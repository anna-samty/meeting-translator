...
model = genai.GenerativeModel(
    ...
    - Format each sentence as: [Original Text] | [Translated Text]
    - If there is only noise or silence, respond with 'SILENCE'.
    """
)

# Initialize Session State for history and duplicate prevention
...
# just_once=True prevents the app from re-processing the same recording on every click
    ...
    if new_id != st.session_state['last_id']:
        ...
        with st.spinner("Translating..."):
            try:
                ...
                # Instruction to handle multiple sentences in one recording segment
                ...
                if "SILENCE" not in res_text.upper():
                    # Handle multiple lines of output if the AI breaks speech into several sentences
